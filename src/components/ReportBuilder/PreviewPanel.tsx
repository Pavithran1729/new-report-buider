...
const cleanWordCountFromDisplay = (text: string): string => {
  ...
  return text
    // Remove entire lines that are just word count
    .replace(/^.*?(?:\*word\*|\*\*word\*\*|word count|words:|word:)\s*\d+.*$/gim, '')
    // Remove lines with format like "**Word Count:** 250"
    .replace(/^.*?\*{1,2}(?:word\s+)?count\*{1,2}.*$/gim, '')
    // Remove lines containing just numbers and "words"
    .replace(/^[\s]*[\*\-]?\s*\d+\s+(?:words?|wc)\s*[\*\-]?\s*$/gim, '')
    // Remove asterisk-wrapped word counts
    .replace(/^\s*\*{1,}\s*\d+\s+(?:words?|wc)\s*\*{1,}\s*$/gim, '')
    // Remove word counts at end of content
...
interface PreviewPanelProps {
  ...
  academicConfig?: any; // Will import type
...
export const PreviewPanel = ({
  ...
  // Clean content for display (remove any word counts)
  const cleanedContent = useMemo(() => cleanWordCountFromDisplay(content), [content]);

  const hasContent = title || cleanedContent;
  ...
  // Detect if two-column layout should be used (research papers)
  const useTwoColumn = academicConfig ? shouldUseTwoColumn(academicConfig.reportType, academicConfig.structure?.layout) : false;
        ...
        {!hasContent ? (
          ...
          <div className="flex h-full">
            ...
            <div className={`flex-1 transition-all duration-300 ${viewModeWidths[viewMode]}`}>
              <PageView title={title} content={cleanedContent} className="h-full" />
            </div>
          </div>
        ) : (
          <div className="flex h-full">
              ...
              <Tabs defaultValue="formatted" className="flex-1 flex flex-col">
                ...
                <TabsContent value="formatted" className="flex-1 overflow-y-auto mt-4 space-y-4" onMouseUp={handleTextSelection}>
                  <motion.div
                    ...
                    className={`${template.styles.colors.background} p-6 rounded-lg transition-all ${useTwoColumn ? 'columns-2 gap-6' : ''}`}
                    style={useTwoColumn ? { columnFill: 'auto', columnRule: '1px solid rgba(255,255,255,0.1)' } : undefined}
                  >
                    {title && (
                      <motion.div
                      ...
                      >
                        {isHighlightMode ? (
                          <HighlightableContent
                            content={cleanedContent}
                            extractedData={extractedDataWithPositions}
                ...
                <TabsContent value="raw" className="flex-1 overflow-y-auto mt-4">
                  <pre className="text-sm bg-muted/30 p-4 rounded-lg whitespace-pre-wrap font-mono">
                    {cleanedContent || "No content generated yet..."}
                  </pre>
                </TabsContent>
              </Tabs>
            </div>
