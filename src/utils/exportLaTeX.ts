...
import { shouldUseTwoColumn } from './formatHelpers';
...
const markdownToLaTeX = (content: string): string => {
  ...
  // Remove horizontal rules (---) - replaced with line spacing
...
export const generateLaTeXDocument = (
  ...
  // Check if two-column layout should be used
  const useTwoColumn = shouldUseTwoColumn(reportType, structure.layout);
  const columnOption = useTwoColumn ? ',twocolumn' : '';
...
\\newpage
\\section*{References}
No citations found in the document. Add citations using [1], [2], etc. in your content.
`;
    }
  }
...
\\documentclass[12pt,a4paper${columnOption}]{${documentClass}}
